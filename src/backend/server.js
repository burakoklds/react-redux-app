...
const dbUrl = `mongodb://${process.env.DB_HOST}/${process.env.DB_NAME + '?ssl=true&replicaSet=moviDB-shard-0&authSource=admin&retryWrites=true&w=majority'}`;
...
mongodb.MongoClient.connect(dbUrl, (err, db) =>  {
  ...
  app.post('/api/movies', (req, res) => {
    ...
    if (isValid) {
      ...
      db.collection('movies').insertOne({ title , cover }, (err, result) => {
        if (err) {
          res.status(500).json({ errors: { global: "Something went wrong" , message: req.body , err}});
        } else {
